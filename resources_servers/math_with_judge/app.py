...
def _normalize_judge_server_url(judge_server_url: str) -> str:
    ...
    if not judge_server_url:
        raise ValueError("math_with_judge config requires non-empty `judge_server_url`.")
...
class LibraryJudgeMathResourcesServer(_OriginalLibraryJudgeMathResourcesServer):
    ...
    def setup_webserver(self):
        ...
        print(
            f"math_with_judge: judge_server_url='{normalized}', judge_model='{self.config.judge_model}' "
            ...
            try:
                async with client.post(url, json=payload, headers=headers, timeout=timeout) as response:
                    if response.status in _RETRYABLE_STATUS_CODES:
                        ...
                        print(
                            f"[WARNING] math_with_judge at attempt={attempt}: judge request "
                ...
                print(
                    f"[WARNING] math_with_judge at attempt={attempt}: judge request failed "
                    f"with error ({exc}); retrying in {_WAIT_SECONDS_BEFORE_RETRY} second(s)..."
                )
                await asyncio.sleep(_WAIT_SECONDS_BEFORE_RETRY + random.uniform(0, 1))
        print(
            f"[WARNING] math_with_judge at attempt={attempt}: Maximum retries reached; "
            "returning empty judge response (verdict will default to NOT EQUAL / score 0.0)."
        )
        return {}
