...
from resources_servers.multichallenge.app import AggregationMode, RubricEvaluation
from resources_servers.multichallenge_outsource.app import (
    MultiChallengeOutsourceConfig,
    MultiChallengeOutsourceServer,
...
def _make_config(**overrides):
    ...
    return MultiChallengeOutsourceConfig(**base)
...
class TestConfigRequiredFields:
    ...
    def test_default_name(self):
        cfg = MultiChallengeOutsourceConfig(
            host="",
            port=0,
            entrypoint="",
            judge_link="0.0.0.0:8000",
        ...
        assert cfg.name == "multichallenge_outsource"

    def test_no_judge_model_server_field(self):
        # The outsource config must not carry the Gym-managed judge_model_server.
        cfg = _make_config()
        assert not hasattr(cfg, "judge_model_server") or "judge_model_server" not in cfg.model_fields


class TestInheritedAggregation:
    """Aggregation logic is inherited unchanged from MultiChallengeServer."""
    ...
    def test_aggregation_mean_inherited(self):
        ...
        server = MultiChallengeOutsourceServer.model_construct(config=cfg, server_client=mock_client)
