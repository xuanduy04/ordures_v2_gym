"""
MultiChallenge Outsource Environment Resources Server.
...
inherited unchanged from :class:`MultiChallengeServer`, so any dataset that is
compatible with ``multichallenge_simple_agent`` only needs its
``agent_ref.name`` changed to ``multichallenge_simple_agent_outsource``.
"""
...
def _fetch_models_json_once(server_url: str) -> dict:
    ...
    ``{"status_code": [...], "text": [...]}`` failure record (mirrors
    ``examples/custom_rewards/utils/llm_judge_utils.fetch_json_once``)."""
...
class MultiChallengeOutsourceConfig(BaseResourcesServerConfig):
...
class MultiChallengeOutsourceServer(MultiChallengeServer):
    """MultiChallenge server that outsources the LLM-judge to an external endpoint."""
            ...
            try:
                async with client.post(url, json=payload, headers=headers, timeout=timeout) as response:
                    if response.status in _RETRYABLE_STATUS_CODES:
                        ...
                        print(
                            f"[WARNING] multichallenge_outsource at attempt={attempt}: judge request "
                ...
                print(
                    f"[WARNING] multichallenge_outsource at attempt={attempt}: judge request failed "
                    f"with error ({exc}); retrying in {_WAIT_SECONDS_BEFORE_RETRY} second(s)..."
                )
                await asyncio.sleep(_WAIT_SECONDS_BEFORE_RETRY + random.uniform(0, 1))
        print(
            f"[WARNING] multichallenge_outsource at attempt={attempt}: Maximum retries reached; "
...
if __name__ == "__main__":
    MultiChallengeOutsourceServer.run_webserver()
